...
func load(config *Config) error {
	bytes, err := os.ReadFile("dazl.yaml")
	if err == nil {
	...
	bytes, err = os.ReadFile(filepath.Join(".atomix", "dazl.yaml"))
	if err == nil {
		return yaml.Unmarshal(bytes, config)
	} else if !os.IsNotExist(err) {
		return err
	...
	if err == nil {
		bytes, err = os.ReadFile(filepath.Join(home, ".atomix", "dazl.yaml"))
		if err == nil {
			return yaml.Unmarshal(bytes, config)
		} else if !os.IsNotExist(err) {
			return err
	...
	bytes, err = os.ReadFile("/etc/atomix/dazl.yaml")
	if err == nil {
		return yaml.Unmarshal(bytes, config)
	} else if !os.IsNotExist(err) {
		return err
